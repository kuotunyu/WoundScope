...
| 2026-08-04 | Hugging Face Space 僅可先建立 deterministic code-only candidate；在 FUSeg 權利人明確確認 derived model weights、ONNX、attribution、可見性與 public non-commercial inference 前，固定為 `PERMISSION_PENDING`，不得建立 model-backed live Space、model repository 或上傳任何 model artifact | Locked |
