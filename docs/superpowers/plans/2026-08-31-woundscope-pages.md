...
2. Read templates, CSS, links, NOTICE, and site-source `LICENSE` through Git objects at `site_source_sha`, not from a dirty worktree. Require `<site_source_sha>:LICENSE` to resolve to blob `6d7d4eed049964731c06b000d257a1bdb2fd6028`, with exact raw size `11,577` bytes and raw SHA-256 `7203278db33515a51443fb4969f84deabc6081086c55a59cc94ee2a384c83f7d`; the byte domain is only the raw bytes returned by `git cat-file blob` for that selected object, copied directly to `LICENSE.txt` with no checkout／worktree read, line-ending normalization, CRLF projection, or reserialization. The `11,782`-byte／`46f4aa5b30f1e3fdec3c30ff381da83fe0323a00d8d7bde8f1a16265c1305fd1` working-copy CRLF variant is diagnostic-only rejected noncanonical content and must never export, validate, hash, or drive budget assertions unless a later central review explicitly changes this lock.
...
> **Amendment 2026-09-03 (owner decision, recorded in chat):** when the owner explicitly delegates this gate, it may be satisfied by a *delegated device-scale-2 review*: each of the three exact engine revisions renders the loopback site at `devicePixelRatio` 2 with the CSS viewport halved for every listed window size and both color schemes (the same reflow geometry as browser-chrome zoom 200%), driven by browser settings rather than keystrokes, on a Linux runner; structured checks (landmark boxes, overlap, horizontal overflow, clipped text, table-scroll operability, footer visibility, skip-link focus ring) plus full-page screenshots are recorded under `manual_browser_zoom_200_percent` with `method: delegated-device-scale-factor-2`, the delegation note, and the runner identity. CSS `zoom` emulation remains excluded and cannot satisfy the gate. The receipt must never describe this as human eyes or menu zoom.
