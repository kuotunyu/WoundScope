...
Then run a case-insensitive path audit over staging and ZIP. Expected forbidden matches: 0 for `.env` / `.env.*`、`.onnx`、`.safetensors`、`.pt`、`.pth`、raster images and exact artifact-directory components such as `artifacts`、`checkpoints`、`gallery`、`sample_predictions` and `tensorboard`. Legitimate Python source module names under `src/` or `app/`, including `src/woundscope/checkpointing.py`, are allowed; source content remains subject to secret and absolute-path validation.

```powershell
.venv\Scripts\python.exe -c "import zipfile; from pathlib import Path,PurePosixPath; root=Path('artifacts/huggingface-space/candidate'); archive=zipfile.ZipFile('artifacts/huggingface-space/WoundScope_hf_space_code_only.zip'); staging=[p.relative_to(root).as_posix() for p in root.rglob('*') if p.is_file()]; zipped=archive.namelist(); directories={'artifact','artifacts','checkpoint','checkpoints','data_manifest','data_manifests','error_gallery','galleries','gallery','sample_prediction','sample_predictions','tensorboard'}; suffixes={'.onnx','.safetensors','.pt','.pth','.bmp','.gif','.jpg','.jpeg','.png','.tif','.tiff','.webp'}; prohibited=lambda name: ((parts:=tuple(part.casefold() for part in PurePosixPath(name).parts)) and (parts[-1]=='.env' or parts[-1].startswith('.env.') or any(part in directories for part in parts[:-1]) or PurePosixPath(name).suffix.casefold() in suffixes)); bad_staging=[n for n in staging if prohibited(n)]; bad_zip=[n for n in zipped if prohibited(n)]; archive.close(); assert not bad_staging and not bad_zip,{'staging':bad_staging,'zip':bad_zip}; print(f'HF_SPACE_FORBIDDEN_ARTIFACT_AUDIT_PASS staging_files={len(staging)} zip_members={len(zipped)} forbidden=0')"
```
